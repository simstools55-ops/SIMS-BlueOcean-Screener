 ...
 * DrivePicker.html remains separate because HtmlService requires an HTML file.
 */
...
function sbosShowOutputSettings() {
  SpreadsheetApp.getUi().alert('v0.1.2では保存先設定欄を用意済みです。Driveフォルダー選択UIは次の実装工程で接続します。');
}
...
function sbosShowDrivePicker() {
  sbosEnsureSheets_();
  const html = HtmlService.createTemplateFromFile('DrivePicker')
    .evaluate()
    .setWidth(760)
    .setHeight(520);
...
function sbosImportDriveFile(fileId) {
  ...
  if (/\.xlsx$/i.test(name)) {
    throw new Error('v0.1.2ではXLSXの直接読込は未実装です。CSV/TSVで保存して選択してください。');
  }
 ...
 * v0.1.2 deliberately does NOT scrape Google Search directly.
...
function sbosEvaluateSerpCandidate_(candidate) {
  ...
  throw new Error('SERP Provider「' + provider + '」はv0.1.2で未実装です。');
}
...
function sbosResumeBatch() {
  ...
  SpreadsheetApp.getUi().alert('現在の処理状態: ' + status + '\n\nv0.1.2ではSERP Provider接続前のため、SERP工程以降の自動再開はまだ行いません。');
}
